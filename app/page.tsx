...
export default function OnlineManipalPage() {
  ...
  // ✅ Fixed handleSubmit — same as LPU working version
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/enquiry", {  // ✅ small 'e'
        method: "POST",
  ...
  return (
    <div className="page-root">
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hero-overlay {
          position: absolute;
          inset: 0;
          background: linear-gradient(120deg, rgba(0,0,0,0.70) 0%, rgba(0,0,0,0.40) 55%, rgba(0,0,0,0.15) 100%);
          z-index: 0;
        }
        .hero-left { flex: 1; min-width: 320px; color: white; position: relative; z-index: 1; }
        .hero-left h1 { font-size: 2.2rem; font-weight: 800; line-height: 1.2; margin-bottom: 20px; margin-top: 20px; }
        .university-badge {
          display: inline-block;
          background: white; color: #e85d26;
          padding: 5px 16px; border-radius: 20px;
          font-size: 0.85rem; font-weight: 700; margin-bottom: 16px;
        }

        .advantage-section {
          background: linear-gradient(90deg, #f4d58d 0%, #eab79a 45%, #e5a88e 100%);
        ...
        .manipal-adv-images {
          display: flex; justify-content: center; align-items: center;
          gap: 30px; flex-wrap: wrap; margin-top: 25px;
        }
        @media (min-width: 768px) {
          .manipal-adv-images { flex-wrap: nowrap; }
        }
        .manipal-img { max-width: 70%; height: auto; border-radius: 12px; }
        .adv-img { height: auto; width: auto; object-fit: contain; }
        @media (min-width: 768px) { .advantage-images { flex-wrap: nowrap; } }
        ...
        .badges-row { display: flex; gap: 0px; flex-wrap: nowrap; }
        .badge-card {
          background: rgba(255,255,255,1); border: 2px solid hsla(0,0%,99%,0.89);
          border-radius: 6px; padding: 16px 20px; text-align: center; width: 95px;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
        }
        .b-icon-img { width: 100px; height: 100px; object-fit: contain; margin-bottom: 4px; }
        .badge-card span { font-size: 10px; line-height: 1.2; color: #000; }

        .ranking-image-wrapper { display: flex; justify-content: center; align-items: center; gap: 40px; flex-wrap: wrap; }
        @media (min-width: 768px) { .ranking-image-wrapper { flex-wrap: nowrap; } }

        .ranking-slider-outer { overflow: hidden; width: 100%; }
        .ranking-slider-track { display: flex; width: max-content; animation: slideInfinite 15s linear infinite; }
        .ranking-slider-track .ranking-image { width: 200px; height: auto; object-fit: contain; margin: 0 24px; flex-shrink: 0; }
        @keyframes slideInfinite {
          0% { transform: translateX(0); }
          100% { transform: translateX(-50%); }
        }
        .ranking-image { height: 310px; width: auto; object-fit: contain; border-radius: 12px; }

        .form-card {
          background: white; border-radius: 12px; padding: 28px;
          width: 340px; color: #222; flex-shrink: 0; margin-top: 20px;
        ...
        .course-card { flex: 0 0 calc(33.333% - 10px); }
        @media (max-width: 900px) { .course-card { flex: 0 0 calc(50% - 8px); } }
        ...
        .testimonial-card { background: white; border-radius: 12px; padding: 24px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); text-align: center; padding: 25px 20px; min-height: 260px; }
        ...
        .testimonial-img { margin-bottom: 18px; }
        .testimonial-img img { width: 90px; height: 90px; border-radius: 50%; object-fit: cover; }
        .testimonial-text { margin-bottom: 15px; font-size: 16px; line-height: 1.6; }
      ...
      <div className="navbar">
        ...
        <button className="btn-primary btn-navbar" onClick={() => openEnquiry()}>
          Apply Now
        </button>
      </div>
      ...
      <div className="hero">
        ...
        <div className="hero-left">
          ...
          <button className="btn-outline" onClick={() => openEnquiry()}>Download Brochure</button>
          <div className="badges-row">
            <div className="block">
              <img src="/muj1.png" alt="NAAC" className="h-auto w-auto max-w-full" />
            </div>
          </div>
        </div>

        {/* ✅ Fixed Form — same as LPU working version */}
        <form className="form-card" onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
          />
          <select name="program" value={formData.program} onChange={handleInput} required>
      ...
      {/* Rankings & Accreditations Slider */}
      <div className="section-bg-white">
        <div className="section">
          ...
          <div className="ranking-slider-outer">
            <div className="ranking-slider-track">
              <img src="/wes_accredation.png" alt="WES" className="ranking-image" />
              <img src="/Accredation2.png" alt="Accreditation 2" className="ranking-image" />
              <img src="/3accredation.png" alt="Accreditation 3" className="ranking-image" />
              <img src="/4accredation.png" alt="Accreditation 4" className="ranking-image" />
              <img src="/5accredation.png" alt="Accreditation 5" className="ranking-image" />
              <img src="/6Accredation.png" alt="Accreditation 6" className="ranking-image" />
              <img src="/accredattion7.png" alt="Accreditation 7" className="ranking-image" />
              <img src="/8accredation.png" alt="Accreditation 8" className="ranking-image" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="section-bg-grey">
        <div className="section">
          ...
          <div className="tabs">
            <button className={`tab ${activeTab === "master" ? "active" : ""}`} onClick={() => handleTab("master")}>Master</button>
            <button className={`tab ${activeTab === "bachelor" ? "active" : ""}`} onClick={() => handleTab("bachelor")}>Bachelor&apos;s</button>
          </div>

          <div className="courses-slider-wrapper">
            <button className="slider-arrow" onClick={() => setIndex((i) => Math.max(i - 1, 0))} disabled={index === 0}>&#8592;</button>
            <div className="courses-slider-outer">
              <div
                className="courses-slider-track"
                style={{ transform: `translateX(calc(-${index * (100 / slidesPerView)}% - ${index * 16}px))` }}
              >
                {courses.map((c, i) => (
                  <div key={i} className="course-card">
                    ...
                    <div className="course-body">
                      ...
                      <button className="btn-enroll" onClick={() => openEnquiry(c.title)}>Enroll Now</button>
                    </div>
                  </div>
                ))}
              </div>
            </div>
            <button className="slider-arrow" onClick={() => setIndex((i) => Math.min(i + 1, maxIndex))} disabled={index === maxIndex}>&#8594;</button>
          </div>
        </div>
      </div>
      ...
      <div className="advantage-section">
        ...
        <div>
          <button className="adv-btn" onClick={() => openEnquiry()}>Download our Brochure</button>
        </div>
      </div>
        ...
        <div className="testimonials-grid">
          {testimonials.map((t, i) => (
            <div key={i} className="testimonial-card">
              <div className="testimonial-img">{t.img}</div>
      ...
      <div className="sticky-contact">
        <button className="sc-btn sc-call" onClick={() => openEnquiry()}>
          <span>📞</span><span>Call Now</span>
        </button>
        <button className="sc-btn sc-chat" onClick={() => openEnquiry()}>
